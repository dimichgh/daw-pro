...
struct MIDISchedulerTests {
    ...
    func basicPlacement() {
        ...
        let events = MIDIEventSchedule.buildEvents(
            clips: [clip], fromBeat: 0, tempoBPM: 120, sampleRate: rate)
        let expected = [
    ...
    func fromBeatSuppressesEarlierOnsets() {
        ...
        let events = MIDIEventSchedule.buildEvents(
            clips: [clip], fromBeat: 2, tempoBPM: 120, sampleRate: rate)
    ...
    func truncationAtClipEnd() {
        ...
        let events = MIDIEventSchedule.buildEvents(
            clips: [spilling], fromBeat: 0, tempoBPM: 120, sampleRate: rate)
        #expect(events.count == 2)
        ...
        #expect(MIDIEventSchedule.buildEvents(
            clips: [atEnd], fromBeat: 0, tempoBPM: 120, sampleRate: rate).isEmpty)

        let pastEnd = midiClip(startBeat: 0, lengthBeats: 2, notes: [
            MIDINote(pitch: 60, velocity: 100, startBeat: 2.5, lengthBeats: 1),
        ])
        #expect(MIDIEventSchedule.buildEvents(
            clips: [pastEnd], fromBeat: 0, tempoBPM: 120, sampleRate: rate).isEmpty)
    }
    ...
    func roundingFloorAtTempoCap() {
        ...
        let events = MIDIEventSchedule.buildEvents(
            clips: [clip], fromBeat: 0, tempoBPM: 400, sampleRate: rate)
        #expect(events.count == 2)
