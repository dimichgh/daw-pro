...
async function capture(tag) {
  const r = await cmd("debug.captureUI", { path: `${OUT}/${tag}.png` });
  if (!r.ok) throw new Error("capture failed " + JSON.stringify(r));
  const buf = readFileSync(`${OUT}/${tag}.png`);
...
for (const [tag, gain] of [["E-gain-minus5_8", -5.8], ["E-gain-plus11_4", 11.4],
  ...
  await sleep(400);
  widthShas.push(await capture(tag));
}
// Every leg must REPAINT (the readout changed) — a frozen sha would mean the
// seed never reached the row and the width legs proved nothing.
check("E2 each match-gain value repaints the strip",
  new Set(widthShas).size === widthShas.length, widthShas.join(" "));
