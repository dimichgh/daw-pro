...
| Reference row + REFERENCE panel | — | **COINCIDENT-EXEMPT** (m22-g) | Mixing against a finished record. The master strip's REFERENCE row shows in BOTH densities and is gated only by whether a reference slot exists — an A/B listening check is beginner-relevant (the stereo-image block's "phone speaker" argument), and its whole content is a name, the MIX\|REF chip, and one number, with nothing advanced to hide. The PANEL is coincident too: every block in it is load-bearing for the single job it does — import/replace/remove, the A/B with its plain-language level-match line, OFFSET/TRIM, the two-curve spectrum, and the delta row. A Simple mode could only hide the spectrum or the deltas, i.e. hide the comparison itself, and anything less than that would change nothing — a **do-nothing toggle, which is worse than none** (the Undo-history / master-strip precedent). The beginner accommodation is made in the COPY instead (a sub-caption under the deltas says "Your mix is 3.4 loudness units quieter than the reference.", and four `ExplainCatalog` entries carry the teaching), not in a second layout. NOT an AI surface — **zero violet**; cyan marks only the earned active REF state and the numeric readouts, amber only the honest ceiling clamp. |
...
| Export dialog | — | **COINCIDENT-EXEMPT** (m23-m3) | The sheet behind the transport EXPORT chip: quality (bit depth), file type, tracks to leave out, and the optional loudness target. Every one of those is a control a beginner exporting their first song needs to at least SEE — "which of these is CD quality" and "give me the version without the vocal" are beginner questions, not Pro ones — and there is nothing a Pro would additionally want that the dialog withholds (stems and the mastered sibling are a different surface, m23-m3c; sample-rate conversion does not exist at any layer). A Simple mode could only hide the loudness target, which is the one control that makes an export sound right on streaming services, or hide the instrumental switch, which is the feature's headline. Anything less than that would change nothing — a **do-nothing toggle, worse than none** (the Undo-history / Reference-panel precedent). The beginner accommodation is made in the COPY instead: each depth carries a plain-language line under its label (`DeliveryFormat.depthDetail`), and one `ExplainCatalog` entry (`.exportDialog`) teaches the card. NOT an AI surface — **zero violet**; cyan marks only the picked option and the numeric readouts, amber only the honest ceiling-clamp notice. |
| Mixer strip INSERTS disclosure + VOL knob | — | **NOT A DENSITY SURFACE** (m23-a) | Two additions to the mixer console that could each be mistaken for a density control, recorded here so nobody "converts" them later. (1) The **INSERTS collapse chevron** is a DISCLOSURE, not a mode: density answers "which controls does this user want to see at all", disclosure answers "do I need the chain on screen *right now* while I ride a fader". They compose rather than compete — the chevron only exists in Pro (the whole inserts section is Pro-only), and folding it does NOT make the strip a Simple strip (sends, output and the count badge stay). It is nonetheless persisted with the SAME doctrine as density: console-wide (the mixer is ONE panel, so strips never carry per-strip chrome state), app-sticky, never project data — `panelLayout.mixerInsertsCollapsed`, the `PanelLayoutStore` sibling of `panelDensity.<panelID>`, **default expanded** so an existing user's chains do not vanish on update. It grows **no chip of its own** (the "never a do-nothing toggle" rule applies to duplicating an existing mechanism too). (2) The round **`VolumeKnob`** renders in **BOTH densities** — a deliberate reversal of the "extra control ⇒ Pro" reflex: volume is the single most beginner-primary control on a strip, and the knob exists precisely because a tall Pro chain shortens the fader, so hiding it in Simple would put it where it is least needed and remove it where a beginner would most benefit. The honest cost is recorded with it: the knob row grows 38 → 52 pt in BOTH modes, so a Simple strip's fader throw shortens by that much (measured to still clear the reserved budget at the window floor by ~130 pt — `MixerStripLayoutTests`). That 52 rather than 65 is the reason the pan readout moved onto its label line: the first cut spent a third text line on it and a full Pro chain stopped fitting at the DEFAULT window, which Simple mode would have paid for too. NOT an AI surface — zero violet; the knob is cyan because volume is cyan everywhere in the app, and the chevron/count badge are neutral chrome. |
